...
def generate_kernel_pod_yaml(keywords):
    ...
    file_path = keywords['j2_file_base']
    j_env = Environment(loader=FileSystemLoader(file_path), trim_blocks=True, lstrip_blocks=True)
...
def launch_kubernetes_kernel(kernel_id, port_range, response_addr, spark_context_init_mode, kernel_name, j2_file_base, envs):
    ...
    keywords['kernel_name'] = kernel_name
    keywords['kernel_spark_context_init_mode'] = spark_context_init_mode
    keywords['j2_file_base'] = j2_file_base
    ...
    for k8s_obj in k8s_objs:
        if k8s_obj.get('kind'):
            if k8s_obj['kind'] == 'Pod':
                ...
                raise Exception("ERROR - Unhandled Kubernetes object kind '{}' found in yaml file - kernel launch terminating!".
                      format(k8s_obj['kind']))
        else:
            raise Exception("ERROR - Unknown Kubernetes object '{}' found in yaml file - kernel launch terminating!".
                      format(k8s_obj))


def launch_kernel(kernel_id, port_range, response_addr, spark_context_init_mode, kernel_name, j2_file_base, envs):
    try:
        logger.warning(f"Starting req for kid {kernel_id}")
        launch_kubernetes_kernel(kernel_id, port_range, response_addr, spark_context_init_mode, kernel_name, j2_file_base, envs)
        return True
    except Exception:
        logger.exception(f"Error in launch_kubernetes_kernel for kid {kernel_id}")
        return False
...
if __name__ == '__main__':
    ...
    launch_kubernetes_kernel(kernel_id, port_range, response_addr, spark_context_init_mode, None, None, {})
